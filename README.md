...
* `script.js` scans the page on `DOMContentLoaded`, restores note text from cookies, and intercepts form submission to save the updated content (plus course name) for one year.
* A single “Export Notes” button is injected and fixed to the bottom-left corner. Clicking it gathers all note text for the detected course ID(s) and downloads a Markdown file such as `bc-gov-essentials-notes.md`.
...
| `noodle.js` | Vanilla JS helper that manages cookies, status messages, and exports. |
