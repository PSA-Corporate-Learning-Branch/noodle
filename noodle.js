...
(function () {
    ...
    function trapFocus(modal) {
        ...
        var focusable = modal.container.querySelectorAll('a[href], button, textarea, input, select, [tabindex]:not([tabindex="-1"])');
        var list = Array.prototype.slice.call(focusable).filter(function (el) {
        ...
        modal.container.addEventListener("keydown", function (event) {
            if (event.key !== "Tab") {
                return;
            }
            if (event.shiftKey) {
    ...
    function makeBackgroundInert(isOpen) {
        var bodyChildren = document.body.children;
        for (var i = 0; i < bodyChildren.length; i++) {
            var el = bodyChildren[i];
            if (el.id === "noodle-notes-overlay") {
                continue;
            }
            if (isOpen) {
    ...
    function createNotesButton() {
        ...
        button.setAttribute("aria-describedby", "noodle-hotkey-hint");

        var hint = document.createElement("span");
        hint.id = "noodle-hotkey-hint";
        hint.className = "visually-hidden";
        hint.textContent = "Press N twice quickly to open Noodle Notes.";
